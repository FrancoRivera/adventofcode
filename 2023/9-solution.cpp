...
int get_prev_value_in_sequence(std::vector<int> sequence) {
  // find the difference between each of the values in the array
  auto diff = std::vector<int>();
  ...
  // curr - prev
  std::cout << "first in diff " << sequence[0] << std::endl;
  return sequence[0] - get_prev_value_in_sequence(diff);
}
...
int part_2() {
  ...
  for (auto l : lines) {
    ...
    sum += get_prev_value_in_sequence(line_ints);
  }
  // test should be 2
  std::cout << "sum of backwards extrapolated: " << sum << std::endl;
  return sum;
}
