...
int main(){
 ...
 for (auto &filter : filters)
        ...
        if (seed_range.start >= filter_range.start && frange_end > seed_range.start)
        {
          ...
          if (frange_end < seed_end){
            ...
            if (seed_end > nstart){
              ...
              if (DEBUG) cout << "    bit after " << nrange2.start << " " << nrange2.length << endl;
              seed_ranges.push_back(nrange2);
            }
          }
          // fully enclosed
          if (frange_end >= seed_end){
            if (DEBUG) cout << " fully" << endl;
       ...
       // check if seed range and frange overlap
        if (seed_range.start < filter_range.start && filter_range.start < seed_end){
          ...
          if(frange_end > seed_end){
            ...
            if (DEBUG) cout << "   nlength: " << nlength << " " << filter_range.dest << endl;
            aux_seed_ranges.push_back(overlap);
            // not overlapped
          ...
          if (seed_end >= frange_end){
            ...
            if (filter_range.start > seed_range.start){
              ...
              if (DEBUG) cout << "   bit before " << prev.start << " " << prev.length << endl; 
              seed_ranges.push_back(prev);
            }
            // get bit after
            if (seed_end > frange_end){
              auto next = range(frange_end, 0.0, seed_end-frange_end);
              if (DEBUG) cout << "   bit after " << next.start << " " << next.length << endl; 
  ...
  {
      if (DEBUG) cout << seed.start << " " << seed.length << endl;
